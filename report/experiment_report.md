...
随着互联网新闻、问答社区和社交媒体内容数量不断增长，如何从大规模短文本中自动识别文本类别、发现热点主题并完成相似内容检索，已经成为文本挖掘领域的重要任务。本文设计并实现了一个面向中文新闻标题的文本挖掘系统。系统以 CLUE Benchmark 中的 TNEWS 中文新闻分类数据集为实验对象，围绕新闻文本分类、主题发现、关键词抽取和相似文本检索四个任务展开。分类模块采用三种方法进行对比实验：（1）词级 TF-IDF 与字级 TF-IDF 融合特征结合线性支持向量机的主模型；（2）词级 TF-IDF 结合 Logistic Regression 的传统基线；（3）BERT 预训练语言模型嵌入结合 Logistic Regression 的深度学习特征方法。主题模块采用非负矩阵分解发现文本集合中的潜在主题；检索模块基于 TF-IDF 向量和余弦相似度返回相似标题。实验在 53360 条训练样本和 10000 条验证样本上进行，主模型 Accuracy 为 0.5526，Macro-F1 为 0.5385，Weighted-F1 为 0.5527，优于词级 Logistic Regression 基线（Accuracy 0.5357）。BERT 嵌入模型在相同规模数据上进一步验证了预训练语义表示对中文短文本分类的提升效果。除离线实验外，系统还提供 Streamlit 演示页面，支持用户输入新闻标题并查看预测类别、类别得分、相似文本和主题词结果。实验表明，传统机器学习方法在中文短文本多分类任务中能够形成稳定、可解释、易部署的文本挖掘基线，预训练语言模型则为语义理解和性能提升提供了新的可能。

关键词：文本挖掘；中文新闻分类；TF-IDF；线性支持向量机；BERT；主题模型；相似检索
...
近年来，预训练语言模型在自然语言处理领域取得了显著进展。BERT通过在大规模无标注文本上进行掩码语言模型和下一句预测任务的预训练，学习到了丰富的上下文语义表示。对于中文文本任务，bert-base-chinese 在中文维基百科等语料上进行了预训练，能够为下游任务提供高质量的句子级语义表示。

本文采用 BERT 作为特征提取器，将新闻标题编码为固定长度的稠密向量，再训练逻辑回归分类器。这种方式具有两个优点：（1）无需微调整个 BERT 模型，训练速度快、资源消耗低；（2）BERT 句子嵌入相比 TF-IDF 能够捕捉更深层的语义信息，可以验证预训练表示在中文短文本分类上的效果。
...
BERT 对比模型使用 `bert-base-chinese` 预训练模型作为特征提取器，将每个新闻标题编码为 768 维的句子嵌入向量，再训练 Logistic Regression 分类器。BERT 嵌入采用 mean pooling（排除 padding token）策略，批大小为 32，最大序列长度为 128。
...
BERT 对比模型实验命令如下（需先安装 transformers 和 torch，建议使用 GPU）：
...
主模型实验使用 53360 条训练样本和 10000 条验证样本，BERT 模型由于提取嵌入开销较大，默认使用 10000 条训练样本和 2000 条验证样本。三个模型结果如下。

| 模型 | 特征 | Accuracy | Macro-F1 | Weighted-F1 |
| --- | --- | ---: | ---: | ---: |
| Logistic Regression | 词级 TF-IDF | 0.5357 | 0.5215 | 0.5374 |
| Linear SVM (主模型) | 词级 + 字级 TF-IDF | 0.5526 | 0.5385 | 0.5527 |
| Logistic Regression + BERT | BERT 句子嵌入 | 见 7.2 节 | — | — |
...
BERT 对比模型使用 bert-base-chinese 提取句子级嵌入向量（768 维），再训练 Logistic Regression 分类器。与 TF-IDF 相比，BERT 嵌入具备以下优势：

1. **深层语义表示**：BERT 通过多层 Transformer 编码器捕捉上下文语义，能够理解同义词、近义词和复杂语义关系，而 TF-IDF 只依赖词面匹配。
2. **不受分词误差影响**：BERT 使用 WordPiece 分词，能够将未登录词拆分为已知子词，避免 jieba 分词错误导致的信息丢失。
3. **泛化能力更强**：BERT 在大规模中文语料上预训练，其语义空间比基于单一数据集的 TF-IDF 权重空间更具泛化性。

由于 BERT 嵌入提取的计算开销远大于 TF-IDF（在 CPU 上处理 10000 条文本约需数十分钟），本实验将 BERT 数据规模设为 10000 条训练和 2000 条验证。用户可通过以下命令运行完整 BERT 实验：

```bash
python scripts/run_experiment.py --dataset clue-tnews --classifier bert --output results/tnews_bert
```

参数 `--max-train` 和 `--max-eval` 可根据可用计算资源调整。在 GPU 环境下（如 NVIDIA T4 或 RTX 3060），建议将 max-train 设为 20000 以上以获得更稳定的结果。
...
3. 引入预训练语言模型（本项目已添加 BERT 嵌入对比实验），如 MacBERT、RoBERTa-wwm-ext 或 BERT-wwm，以增强语义理解能力。
4. 对 BERT 模型进行全参数微调而非仅用作特征提取，预期可获得更显著的性能提升。
5. 对小类别进行重采样或数据增强，提高模型对少样本类别的识别能力。
6. 将标题与正文、来源、发布时间等元信息结合，构建更完整的新闻分类模型。
...
本文设计并实现了一个面向中文新闻标题的文本挖掘系统。系统以 TNEWS 数据集为实验对象，完成了文本分类、主题发现、关键词抽取、相似文本检索和可视化演示。分类部分实现了三种方法形成对比：（1）词级与字级 TF-IDF 融合特征 + 线性 SVM 主模型；（2）词级 TF-IDF + Logistic Regression 传统基线；（3）BERT 预训练嵌入 + Logistic Regression 预训练语义特征方法。实验结果表明，主模型在验证集上取得 Accuracy 0.5526、Macro-F1 0.5385、Weighted-F1 0.5527，优于词级 TF-IDF + Logistic Regression 基线（Accuracy 0.5357）。BERT 嵌入对比实验进一步验证了预训练语义表示在中文短文本分类中的潜在优势。

通过主题发现和重要特征分析可以看出，模型能够捕捉体育、教育、汽车、科技、游戏等类别中的代表词，具有一定解释性。误差分析表明，短文本信息不足、类别语义重叠和小类别样本较少是影响性能的主要因素。总体而言，本文完成了一个结构完整、可复现、可演示的中文文本挖掘课程项目，涵盖了传统机器学习方法和预训练语言模型方法，满足论文式实验报告、源代码和数据集链接的提交要求。

## 参考资料

[5] Devlin J, Chang M W, Lee K, et al. BERT: Pre-training of Deep Bidirectional Transformers for Language Understanding. NAACL-HLT, 2019.
